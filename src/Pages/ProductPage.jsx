...
const ProductPage = () => {
  ...
  const { title, type, searchQuery = "" } = location.state || {}; // Avoid undefined searchQuery
  ...
  useEffect(() => {
    if (!searchTerm) {
      setFilteredData(equipmentData); // Show all products if no search term
      return;
    }
    setFilteredData(
        ...
        <div className="mb-4 flex justify-start ml-0">
          <div className="relative w-full md:w-1/4 ml-14">
            ...
            <input
              ...
              className="p-2 pl-10 border-2 border-gray-300 rounded-lg w-full focus:border-blue-500 focus:outline-none"
              value={searchTerm}
